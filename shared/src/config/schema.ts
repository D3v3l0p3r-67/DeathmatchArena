...
function gameModeFields(): FieldDescriptor[] {
  ...
  return [
    ...
    number("campaign.enemyMoveSpeedMultiplier", MODES, "Campaign", "Enemy move speed x", "Scales every campaign enemy's walk and run speed. The game-mode layer of the campaign tuning hierarchy; difficulty, level, type and instance layers multiply on top. Multiplayer ignores it.", { min: 0.1, max: 2, step: 0.05 }),
    number("campaign.enemyProjectileSpeedMultiplier", MODES, "Campaign", "Enemy projectile speed x", "Scales everything a campaign enemy launches: bullets, rockets and thrown grenades. Lower is easier to dodge.", { min: 0.1, max: 2, step: 0.05 }),
    number("campaign.enemyFireRateMultiplier", MODES, "Campaign", "Enemy fire rate x", "Scales campaign enemies' shots per second.", { min: 0.1, max: 2, step: 0.05 }),
    number("campaign.enemyReactionTimeMultiplier", MODES, "Campaign", "Enemy reaction time x", "Scales how long a campaign enemy takes to notice and engage. Above 1 reacts slower, i.e. easier.", { min: 0.5, max: 3, step: 0.05 }),
